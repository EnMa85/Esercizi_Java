...
<body>
	...
	%>
		...
		<p>
			<br> <br>E' stato inserito il seguente prodotto:<br> <br>
		</p>

		<p>${ prodotto.getNome() }- Prezzo €: ${ String.format("%.2f", prodotto.getPrezzo()) }
			- Quantita': ${ prodotto.getQuantita() }</p>
		<%      	
		        	break;
		...
		<c:if test="${listaModifiche != null}">
			...
			<ol>
				<c:forEach var="prodotto" items="${ listaModifiche }">
					<li>${ prodotto.getNome() }- Prezzo €: ${ String.format("%.2f", prodotto.getPrezzo()) }
						- Quantita': ${ prodotto.getQuantita() }</li>
				</c:forEach>
			</ol>
		</c:if>
		<%	        	
	...
	<div id="footer">
		<a href="GestoreServlet"><button class="button" id="back">Torna
				indietro</button></a> <a href="LogoutServlet"><button class="button"
				id="back">Esci</button></a>
	</div>
